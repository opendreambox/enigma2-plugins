	...
	def __init__(self, session):
		...
		ConfigListScreen.__init__(
			...
			[
				...
				getConfigListEntry(_("Remove not existing events"), config.plugins.autotimer.check_eit_and_remove, _("Check the event id (eit) and remove the timer if there is no corresponding EPG event. Not compatible with SerienRecorder and IPRec.")),
			],
			session = session,
