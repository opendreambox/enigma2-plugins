...
from AutoTimerAddon import AutoTimerAddonEntry, addonDefinitions, AT_EXCLUDE, AT_INCLUDE, AT_EXTENSION, \
		atdefaultFilters,ataddonXMLtypes, ataddonXMLignoreKeys, ataddonSeriesRenameKey
...
def parseConfig(configuration, list, ignoreList, version = None, uniqueTimerId = 0, defaultTimer = None):
	try:
		...
		atLog( ATLOG_WARN, 'Config version "%s" is not a valid integer, assuming old version' % version)
		intVersion = -1
...
def parseEntry(element, baseTimer, defaults = False):
	if not defaults:
		...
		if not baseTimer.match:
			atLog( ATLOG_WARN, 'Erroneous config is missing attribute "match", skipping entry')
			return False

		# Read out name
		baseTimer.name = element.get("name", "").encode("UTF-8")
		if not baseTimer.name:
			atLog( ATLOG_INFO, 'Timer is missing attribute "name", defaulting to match')
		...
		if enabled == "no":
			...
			atLog( ATLOG_WARN, 'Erroneous config contains invalid value for "enabled":', enabled,', disabling')
			baseTimer.enabled = False
	...
	if l:
		...
		for afterevent in l:
			...
			if value in idx:
				...
				atLog( ATLOG_WARN, 'Erroneous config contains invalid value for "afterevent":', afterevent,', ignoring definition')
				continue
	...
	if l:
		atLog( ATLOG_DEBUG, "parseEntry: Found %d addons" %len(l) )						
		for addonsXml in l:			
			for type,typeName in iteritems(ataddonXMLtypes):
				addons = {}
				addonList = addonsXml.findall(typeName)
				if addonList:
					atLog( ATLOG_DEBUG, "parseEntry: adding addons for type %s" %( typeName ) )						
					for addon in addonList:
						curAddon = parseAddon(addon, type)
						if curAddon:
							addAddonEntry( baseTimer, type, curAddon )

	if baseTimer.series_labeling:
		seriesAddonKey = ataddonSeriesRenameKey
		seriesParamValues = {"enabled": True}
		seriesAddon = AutoTimerAddonEntry( addonKey = seriesAddonKey, addonType = AT_EXTENSION, description = '', paramValues = seriesParamValues ) 
		addAddonEntry( baseTimer, AT_EXTENSION, seriesAddon )
		
	# Read out exclude
	l = element.findall("exclude")
	if l:
		atLog( ATLOG_DEBUG, "parseEntry: Found %d excludes" %len(l) )						
		excludes = ([], [], [], [])
		for exclude in l:
			...
			if where in atdefaultFilters:
				valueEncoded = value.encode("UTF-8")
				excludes[atdefaultFilters[where]].append( valueEncoded )
				# Add excludes to addons (for addon/filter check)
				excludeAddon = getAddonFromFilter( where, AT_EXCLUDE, valueEncoded )
				if excludeAddon:
					addAddonEntry( baseTimer, AT_EXCLUDE, excludeAddon )
	...
	if l:
		atLog( ATLOG_DEBUG, "parseEntry: Found %d includes" %len(l) )						
		includes = ([], [], [], [])
		for include in l:
			...
			if where in atdefaultFilters:
				valueEncoded = value.encode("UTF-8")
				includes[atdefaultFilters[where]].append( valueEncoded )
				# Add includes to addons (for addon/filter check)
				includeAddon = getAddonFromFilter( where, AT_INCLUDE, valueEncoded )
				if includeAddon:
					addAddonEntry( baseTimer, AT_INCLUDE, includeAddon )
...
def addAddonEntry( timer, type, addonEntry):
	addonEntries = timer.addonEntries
	addonKey = addonEntry.addonKey
	typeEntries = addonEntries.get(type,{})
	keyEntries = typeEntries.get(addonKey,[])
	keyEntries.append(addonEntry)
	typeEntries[addonKey] = keyEntries
	timer.addonEntries[type] = typeEntries

def getAddonFromFilter( where, type, value ):
	filterAddonKey = "AutoTimer." + where
	paramValues = {"searchstring": value}
	filterAddon = AutoTimerAddonEntry( addonKey = filterAddonKey, addonType = type, description = '', paramValues = paramValues )
	return filterAddon
...
def parseEntryIgnore(element, ignoreEntry):
	...
	if not ignoreEntry.serviceref:
		atLog( ATLOG_WARN, 'Erroneous config is missing attribute "serviceref", skipping ignore entry')
		return False
	ignoreEntry.eit = element.get("eit", "").encode("UTF-8")
	if not ignoreEntry.eit:
		atLog( ATLOG_WARN, 'Erroneous config is missing attribute "eit", skipping ignore entry')
		return False
...
def parseConfigOld(configuration, list, uniqueTimerId = 0):
	atLog( ATLOG_INFO, "Trying to parse old config")
	...
	for timer in configuration.findall("timer"):
		...
		if not name:
			atLog( ATLOG_WARN, 'Erroneous config is missing attribute "name", skipping entry')
			continue
		...
		if match:
			...
			if not match:
				atLog( ATLOG_WARN, 'Erroneous config contains empty attribute "match", skipping entry')
				continue
		# V2-
		else:
		...
		if enabled:
			if enabled == "no":
				...
				atLog( ATLOG_WARN, 'Erroneous config contains invalid value for "enabled":', enabled,', skipping entry')
				enabled = False
		# V1
		else:
		...
		# V3-
			...
			if Len:
				...
				if start and end:
					...
					atLog( ATLOG_WARN, 'Erroneous config contains invalid definition of "timespan", ignoring definition')
					timetuple = None
			else:
				timetuple = None
...
def buildConfig(defaultTimer, timers, ignores, webif = False):
	...
	# Addons
	if defaultTimer.addonEntries:
		extend( getAddonXML(defaultTimer.addonEntries) )
	...
	for timer in timers:
		...
		# Addons
		if timer.addonEntries:
			extend( getAddonXML(timer.addonEntries) )
...
def getAddonXML( addonList ):
	returnList = ["  <addons>\n"]
	addonFound = False
	for addonType,addonTypeEntries in iteritems(addonList):
		addonString = ataddonXMLtypes[addonType]
		atLog( ATLOG_DEBUG, "getAddonXML: create addons for type %s" %( addonString ) )
		for addonKey,addonEntries in iteritems(addonTypeEntries):
			atLog( ATLOG_DEBUG, "getAddonXML: create addons for addonKey '%s'" %( addonKey ) )
			for addonEntry in addonEntries:
				try:
					if not addonKey in ataddonXMLignoreKeys:					
						atLog( ATLOG_DEBUG, "getAddonXML: create addonEntry %s, %s" %( addonKey, addonEntry.description ) )
						returnList.extend(('	<' + addonString + ' where="', stringToXML(addonEntry.addonName), '" domain="', stringToXML(addonEntry.addonDomain), '" ',
								'description="', stringToXML(addonEntry.description), '">\n'))
						for name,value in iteritems(addonEntry.paramValues):
							if name and value:
