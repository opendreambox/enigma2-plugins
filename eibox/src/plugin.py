	...
	def parseMultiRead(self, knxdata, user_args):
		...
		try:
			...
			for node in dom.childNodes[0].childNodes:
				if node.nodeType == xml.dom.minidom.Element.nodeType and node.tagName == 'objects':
					for subnode in node.childNodes:
						if subnode.nodeType == xml.dom.minidom.Element.nodeType and subnode.tagName == 'object':
							i = 0
							object_id = None
							value = None
	...
	def loadXML(self, filename):
		try:
			...
			for node in projectfiledom.childNodes[0].childNodes:
				if node.nodeType == xml.dom.minidom.Element.nodeType and node.tagName == 'zones':
					for subnode in node.childNodes:
						if subnode.nodeType == xml.dom.minidom.Element.nodeType and subnode.tagName == 'zone':
