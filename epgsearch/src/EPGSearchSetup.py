...
class EPGSearchSetup(Screen, ConfigListScreen):
	...
	def buildConfig(self):
		...
		self.list.append( getConfigListEntry(_("Add \"Search EPG\" to Channel Selection"), config.plugins.epgsearch.channellist_redbutton, _("Enable to add \"Search EPG\" to Channel Selection red button.")) )
		...
		if autoTimerAvailable:
			self.list.append( getConfigListEntry(_("History list"), config.plugins.epgsearch.blue_function, _("Select history view (default = text search history and search filter list)")) )
